...
docker run -v "$(pwd)"/:/tesseract --rm tesseract-demo bash -c "cd kill-count-rois && conda run -n \"tesseract\" tesseract imgs.txt results --dpi 70 --psm 8 -c tessedit_char_whitelist=0123456789 tsv"